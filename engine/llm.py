...
class LanguageModel:
    ...
    def pick(self, prefix, candidates, yougen=-1, yougen_shrunk='', yougen_yomi='') -> dict[int, str]:
        ...
        encoded_candidates = self._tokenizer(inputs, padding=True)
        ...
        if 0 <= yougen and encoded_candidates.input_ids[pos_yougen][mask_token_index] == self._tokenizer.unk_token_id:
            if yougen_yomi in self._yougen_tokens:
                yp = sum(probabilities[self._yougen_tokens[yougen_yomi]].tolist())
            else:
                yp = 0.0
        ...
        for i, ids in enumerate(encoded_candidates.input_ids):
            if ids[mask_token_index + 1] in (self._tokenizer.sep_token_id, self._tokenizer.pad_token_id):
                continue
            ...
            with torch.no_grad():
                p = self._model(**encoded_input).logits[0, mask_token_index + 1 - offset]
            p = torch.nn.functional.softmax(p, dim=0)

            if 0 <= yougen and i == pos_yougen and ids[mask_token_index + 1] == self._tokenizer.unk_token_id:
                if yougen_yomi in self._yougen_tokens:
                    probabilities[i] *= sum(p[self._yougen_tokens[yougen_yomi]].tolist())
                else:
                    probabilities[i] = 0.0
            else:
